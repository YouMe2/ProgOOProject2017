...
public class Scoreboard extends GameCompound {
	...
	public Scoreboard(Player player) {
		...
		stepsDisplay = new Background("stepbar", 0, Math.round(LHPIXEL_HEIGHT * 0.1f), 0,
				Math.round(LHPIXEL_HEIGHT * 0.8f));
		stepsDisplay.setLayer(Viewable.SB_LAYER);

		for (int i = 0; i < pointsDisplay.length; i++) {
			pointsDisplay[i] = new Background(null, LHPIXEL_WIDTH * (13 + 3 * i), LHPIXEL_HEIGHT, LHPIXEL_WIDTH * 2,
					LHPIXEL_HEIGHT);
		...
		for (int i = 0; i < lifesDisplay.length; i++) {
			lifesDisplay[i] = new Background(null, LHPIXEL_WIDTH * (0 + 3 * i), LHPIXEL_HEIGHT, LHPIXEL_WIDTH * 2,
					LHPIXEL_HEIGHT);
